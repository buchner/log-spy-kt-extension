...
class InterceptablePrintStreamProperty : FreeSpec() {
    ...
    init {
        fun compareResultsOf(actions: (PrintStream) -> Unit) {
            ...
            val interceptable = InterceptablePrintStream(PrintStream(forwarded), ByteArrayOutputStream())
        ...
        val arbAction: Arb<PrintStreamAction> = arb { rs ->
            sequence {
                val actionNameGenerator = Arb.enum<PrintStreamActionName>().generate(rs).iterator()
                ...
                val charSequenceGenerator = Arb.string().map { it as CharSequence }.generate(rs).iterator()
                val charGenerator = Arb.char().generate(rs).iterator()
                val byteArrayGenerator = Arb.string().map { it.toByteArray() }.generate(rs).iterator()
                val charArrayGenerator = Arb.string().map { it.toCharArray() }.generate(rs).iterator()
                while (true) {
                    ...
                    val result: PrintStreamAction = when (actionNameGenerator.next().value) {
                        PRINT_INT -> PrintStreamAction("print int $intValue") { print(intValue) }
                        PRINT_DOUBLE -> PrintStreamAction("print double $doubleValue") { print(doubleValue) }
                        PRINT_LONG -> PrintStreamAction("print long $longValue") { print(longValue) }
                        PRINT_FLOAT -> PrintStreamAction("print float $floatValue") { print(floatValue) }
                        PRINT_STRING -> PrintStreamAction("print string $stringValue") { print(stringValue) }
                        PRINT_BOOLEAN -> PrintStreamAction("print boolean $booleanValue") { print(booleanValue) }
                        ...
                        WRITE_BYTE_ARRAY -> PrintStreamAction("write byte array $byteArrayValue") { write(byteArrayValue) }
                        WRITE_BYTE_ARRAY_WITH_OFFSETS -> PrintStreamAction("write byte array $byteArrayValue with offsets") {
                            write(
                                byteArrayValue,
                                0,
                                byteArrayValue.size
                        ...
                        PRINTLN_INT -> PrintStreamAction("println int $intValue") { println(intValue) }
                        PRINTLN_DOUBLE -> PrintStreamAction("println double $doubleValue") { println(doubleValue) }
                        PRINTLN_LONG -> PrintStreamAction("println long $longValue") { println(longValue) }
                        PRINTLN_FLOAT -> PrintStreamAction("println float $floatValue") { println(floatValue) }
                        PRINTLN_STRING -> PrintStreamAction("println string $stringValue") { println(stringValue) }
                        PRINTLN_BOOLEAN -> PrintStreamAction("println boolean $booleanValue") { println(booleanValue) }
                        PRINTLN_CHAR_ARRAY -> PrintStreamAction("println char array $charArrayValue") {
                            println(
                                charArrayValue
                            )
                        }
                        PRINTLN_OBJECT -> PrintStreamAction("println object") { println(objectValue) }
                        APPEND_CHAR -> PrintStreamAction("append char $charValue") { append(charValue) }
                        APPEND_CHAR_SEQUENCE -> PrintStreamAction("append char sequence $charSequenceValue") {
                            append(
                                charSequenceValue
                            )
                        }
                        APPEND_CHAR_SEQUENCE_WITH_RANGE -> PrintStreamAction("append char sequence $charSequenceValue with range") {
                            append(
                                charSequenceValue,
                                0,
                        ...
                        FORMAT -> PrintStreamAction("format int $intValue") { format("%d", intValue) }
                        PRINTF -> PrintStreamAction("printf int $intValue") { printf("%d", intValue) }
        ...
        val arbActions: Arb<PrintStreamActions> = arb { rs ->
            sequence {
                ...
                while (true) {
                    ...
                    val actions = actionGenerator.asSequence().take(numberOfActions).map { it.value }.toList()
                    yield(PrintStreamActions(actions))
                }
            }
        }
        "actions are transparent" - {
            checkAll(arbActions) { actions ->
                compareResultsOf(actions)
            }
        }
    }
    ...
    private class PrintStreamActions(private val actions: List<PrintStreamAction>): (PrintStream) -> Unit {
    ...
    private class PrintStreamAction(private val description: String, private val action: PrintStream.() -> Unit) :
            (PrintStream) -> Unit {
        override fun invoke(stream: PrintStream) {
