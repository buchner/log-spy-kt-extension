 ...
 *
 * Unlike [io.kotest.property.arbitrary.StringShrinker] the shrinking happens independent of the
 * position of the codepoints in the string.
 *
...
class PositionIndependentStringShrinker(private val minLength: Int = 0) : Shrinker<String> {
    ...
    private fun shrinksByLeftOutCodepoints(value: String): List<String> {
        ...
        val withoutCodepoints =
            value.codePoints()
                .asSequence()
                .map { filter ->
                    value.codePoints()
                        .asSequence()
                        .filterNot { codepoint -> codepoint == filter }
                        .fold(StringBuilder(), StringBuilder::appendCodePoint)
                        .toString()
                }
                .filter { string -> string.length >= minLength }
                .toList()
        val withReducedCodepoints =
            (0..value.codePointCount(0, value.length)).asSequence()
                .map { filter ->
                    value.codePoints()
                        .asSequence()
                        .withIndex()
                        .filterNot { it.index == filter }
                        .map { it.value }
                        .fold(StringBuilder(), StringBuilder::appendCodePoint)
                        .toString()
                }
        ...
        return (withoutCodepoints + withReducedCodepoints).distinct().sortedBy { it.length }
    }
    ...
    private fun shrinksByReducedSetOfCodepoints(value: String): List<String> {
        ...
        return if (minFrequency == null || maxFrequency == null || minFrequency == maxFrequency) {
            ...
            val cutoffMaxFrequency = max(maxFrequency * 0.9, (minFrequency + 1).toDouble())
            val cutoffMinFrequency = min(minFrequency * 1.3, (maxFrequency - 1).toDouble())
            val toBeReplaceds =
                codepointFrequencyByCodepoint.asSequence()
            ...
            val replacements =
                codepointFrequencyByCodepoint.asSequence()
                    ...
                    .map { entry -> entry.key }
            toBeReplaceds
                .flatMap { toBeReplaced ->
                    replacements.map { replacement ->
                        value.codePoints()
                            .asSequence()
                            .map { codepoint ->
                                if (codepoint == toBeReplaced) replacement else codepoint
                            }
                            .fold(StringBuilder(), StringBuilder::appendCodePoint)
                            .toString()
                    }
